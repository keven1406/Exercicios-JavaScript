...
const average = array => plus => array.reduce(plus)/array.length
const plus = (a,b) => a + b 
const age = p => p.died - p.born
const male = p => p.sex == "m"
const female = p => p.sex == "f"

console.log(average(ancestry.filter(male).map(age))(plus))
console.log(average(ancestry.filter(female).map(age))(plus))
